#!/bin/bash

GIT_URL=$1
GIT_COMMIT=$2
GIT_BRANCH=$3
JOB_NAME=$4
USE_TAG=$5

echo "STATIC BUILD SCRIPT"
echo "GIT_URL=${GIT_URL}"
echo "GIT_COMMIT=${GIT_COMMIT}"
echo "GIT_BRANCH=${GIT_BRANCH}"
echo "JOB_NAME=${JOB_NAME}"
echo "USE_TAG=${USE_TAG}"


image=`echo ${JOB_NAME} | awk -F'.' '{print $5}'`

echo $image

######


# create fastbuild directory
mkdir fastbuild
cd fastbuild


# cleanup and get fresh code
rm -fR wzu-docker
git clone ${GIT_URL}
cd wzu-docker
git checkout "${GIT_BRANCH}"

GIT_COMMIT_BEFORE_LAST=`git log --pretty=format:"%H" |head -2 | tail -1`
echo "GIT_COMMIT_BEFORE_LAST=${GIT_COMMIT_BEFORE_LAST}"

# if we're not on a feature branch, we want to find the pull request
tag=`basename $GIT_BRANCH`
echo "tag=${tag}"
if  [[ $GIT_BRANCH != *feature* ]]
then
    pr="`python ../../fastbuild_scripts/extract_open_pull_request_id.py "refs/heads/${tag}" ${GIT_COMMIT_BEFORE_LAST}`"
    echo "pr=${pr}"

    # validate id
    if [[ ${pr} =~ ^-?[0-9]+$ ]]
    then
        echo "pr=${pr} is valid!"
        tag=${pr}
    else
        echo "pr=${pr} is NOT valid!"
        exit -1
    fi
else
    # feature branch: build only if we have changes in a docker module!
    echo "feature branch"
fi

echo "TAG=${tag}"

# define registry to push to
# feature -> registry-t
# branch develop -> registry-i
# branch master -> registry
# (pattern matching in case statements: http://docstore.mik.ua/orelly/unix3/upt/ch35_11.htm)
branch=`basename $GIT_BRANCH`
registry="registry-t.sbb.ch"
case $branch in
  *master)
    registry="registry.sbb.ch"
  ;;
  *develop)
    registry="registry-i.sbb.ch"
  ;;
  *)
    registry="registry-t.sbb.ch"
  ;;
esac

# call build_and_push
pwd
echo "build_and_push.sh ${registry} ${image} ${tag} ${USE_TAG}"
../../fastbuild_scripts/fastbuild_and_push.sh ${registry} ${image} ${tag} ${USE_TAG}
