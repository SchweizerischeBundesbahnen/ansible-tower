...
filesTouched=`git show --pretty="format:" --name-only develop..${GIT_BRANCH} | perl -lne 'print tr:/::, " $_"' | sort -n | uniq | grep -v '^$' | cut -d' ' -f2 |  grep -v -E ".git|_doc|_scripts"`
...
do
    ...
    # Since 'base' is also part of 'jenkins-slave-java' etc, regex needs some complexity..., see http://stackoverflow.com/questions/9155590/regexp-match-character-group-or-end-of-line
