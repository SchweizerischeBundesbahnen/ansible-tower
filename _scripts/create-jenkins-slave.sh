...
sudo docker run --privileged -d -v "$externalfshome/$randomint/mavenrepo:/var/data/jenkins/m2/repository" -v "$externalfshome/$randomint/tmp:/tmp" -v "$externalfshome/$randomint/workspace:/var/data/jenkins/workspace" -v "$externalfshome/$randomint/.jenkins:/var/data/jenkins/.jenkins" -e master=$master -e executors=1 -e ciuser=fsvctip -e cipassword=sommer11 -e labels=$labels --name $imagename-$randomint ${registry}/${imagename}:${tag}
