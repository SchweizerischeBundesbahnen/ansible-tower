...
function create_android_container() {
  sudo docker run \
    --privileged \
    -d ${REPO_LINK} \
    -p ${randomint}:${randomint} \
    --memory=${android_memory_limit} \
    -e master=${master} \
    -e executors=${execount["$labels"]} \
    -e ciuser=fsvctip \
    -e cipassword=sommer11 \
    -e slavename=${slavename} \
    -e externalport=${randomint} \
    -e host=$HOSTNAME \
...
function create_privileged_container() {
  sudo docker run \
    ...
    -e host=$HOSTNAME \
    -e additional_args="${additional_args}" \
    --name $containername ${registry}/${imagename}:${tag}
}
