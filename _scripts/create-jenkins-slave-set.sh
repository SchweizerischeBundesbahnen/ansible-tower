...
function checkOrStartVarnish() {
  ...
  if [ ${_varnishCount} -lt 1 ]; then
	...
	sudo docker run -p 80:80 -n repocache -v /etc/wzu/jenkins-varnish-config:/data/varnish --env 'VCL_CONFIG=/data/varnish/repo.sbb.ch.vcl' --env 'CACHE_SIZE=20g' registry.sbb.ch/kd_wzu/varnish
	if [ $? -ne 0 ]; then
